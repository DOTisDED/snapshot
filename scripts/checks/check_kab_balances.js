...
import kab_balances from "./kab-balances.json"// assert {type: "json"};
...
async function checkBalances() {
  ...
  let converted_kab_balances = convertAddresses(kab_balances, "AccountId");
  let converted_crowdloan = crowdloan;//convertAddresses(crowdloan);

  try {
	const api = kab_balances;
	...
	for (let account in converted_balances) {
	  ...
	  //const fetched_balances = kab_balances
	  let fetched_balance = converted_kab_balances[account]
	  ...
	  if (Math.abs(got - expected) > 1000 && got != 0) {
		i += 1
		...
		//await (api.tx.balances.setBalance(address, free_balance.toString(), reserved_balance.toString())).signAndSend(sudo, { nonce: -1 })
		//await (api.tx.relaySchedule.schedule(api.tx.balances.forceUnreserve(address, reserved_balance.toString()))).signAndSend(sudo, { nonce: -1 })
		await fs.appendFile('kab-mismatch.json', err);
	  }
	...
	console.log(i + " account with a difference")
