...
// 	//leftint = parseInt(dm.div.toString()) //+ dm.mod.toString());
// 	rightint = parseInt("." + dm.mod.toString());
...
function toUnit(balance, decimals) {
    base = new BN(10).pow(new BN(decimals));
    dm = new BN(balance).divmod(base);
    return parseFloat(dm.div.toString() + "." + dm.mod.toString().padStart(decimals,'0'))
}
...
async function getTotalIssuance() {
	try {
		...
		let total_issuance = toUnit(await api.query.balances.totalIssuance(),config.decimals)
		...
		console.log("writing")
		await fs.writeFile('kab-issuance.json', JSON.stringify(data));
...
async function takeSnapshot() {
	try {
		...
		for (account of all_accounts) {
			...
			balances[address] = {
				"AccountId": address,
				"Free": toUnit(free,config.decimals),
				"Reserved": toUnit(reserved,config.decimals),
				"Total": toUnit(free.add(reserved),config.decimals),
			};

		}

		console.log("writing")
		await fs.writeFile('balances.json', JSON.stringify(balances));
		process.exit(0)
	} catch (error) {
		console.log(error)
		process.exit(1)
